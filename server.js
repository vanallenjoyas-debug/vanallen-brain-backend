...
app.post('/api/sync-posts', async (req, res) => {
  try {
    ...
    for (const post of allPosts) {
      ...
      // manual_copy NO se toca en sync
      await pool.query(`
...
app.get('/api/posts', async (req, res) => {
  try {
    ...
    if (date_to) where.push(`timestamp <= '${date_to}'`);
...
// GUARDAR COPY MANUAL — nunca se borra con sync
...
app.post('/api/analyze-patterns', async (req, res) => {
  try {
    const rows = await pool.query(
      'SELECT caption, manual_copy, like_count, comments_count, reach, saved, comments_data FROM va_posts WHERE excluded=FALSE ORDER BY like_count DESC LIMIT 100'
    );
    ...
    const msg = await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `Sos un analista de contenido para Van Allen Joyas, marca de joyeria artesanal argentina con simbolismo vikingo, celta, wicca y esoterico.
...
app.post('/api/generate-copy', async (req, res) => {
  try {
    ...
    const rows = await pool.query(
      'SELECT caption, manual_copy, like_count FROM va_posts WHERE excluded=FALSE ORDER BY like_count DESC LIMIT 15'
    );
    ...
    const msg = await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `Sos el copywriter de Van Allen Joyas. Joyeria artesanal argentina con simbolismo vikingo, celta, wicca y esoterico.

VOZ: segunda persona informal (vos), evocador, mistico con peso historico, sin autoproclamar calidad, sin marketing generico.

REFERENCIA DE VOZ (posts que mejor funcionaron):
${referencias}
...
app.get('/api/comments', async (req, res) => {
  try {
    ...
    const rows = await pool.query(
      `SELECT id, caption, like_count, timestamp, comments_data FROM va_posts WHERE ${where.join(' AND ')} ORDER BY ${orderBy}`
    );
...
app.post('/api/comments/analyze', async (req, res) => {
  try {
    ...
    const msg = await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `Analista de contenido de Van Allen Joyas (joyeria esoterica argentina).

Analiza estos comentarios y extrae insights:
